...
+ `pander`, for printing formatted R output
...
**Michael**: Update the workflow below
...
AnalysisDF %>% group_by(ArrestDate) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x = ArrestDate, y=n))+
  ...
  geom_smooth(color="black", span=0.1)+
  xlab('')+
  ylab('Arrests')+
...
2019 has incomplete data, let's exclude it for now. **Michael**: Fix the code below so that 2019 is included. 
...
**Michael**: Fix the title to the plot below.
...
By looking at how age group-specific rates vary over time, we can take advantage of observable instability that may exist in those rates to identify potential problems in our statistical analyses. 
...
To finish exploring explanatory variables, we'll examine arrest counts and rates by weekday.
...
Now that we have a general idea of how our outcome varies across explanatory variables, let's examine the distribution of the outcome, arrest counts.
...
**Michael** Need to add a small paragraph here summarizing what we accomplished in this section. 
...
Now that we've created several models, we can compare their appropriateness with by comparing their Akaike's Information Criterion (AIC) values.

We can use `kable()` from the `kableExtra` package to build a more useful table. The table we create indicates that Model F1 is the most appropriate of the 6 models we examined. 
...
The plot we've created shows that a small number of points is impacting our model.
...
It appears that a few outlier points may be very influential to our model. Let's revisit some of the exploratory plots we created.

It appears that we have very few observations for our two oldest age groups. 
...
We now have more observations for our oldest group. 
...
Let's evaluate the Cook's distance value of points to determine whether this is of systematic or spurious origin. 

We create a dataframe with an identification number for each point and each point's corresponding Cook'S distance value. This is appended to our original dataframe.
...
Now that we've updated our dataframe to include the Cook's distance value of each observation, we can determine whether our influential observations are systematic and which factors explain systematic influence over our model. 

Creating an independent plot of Cook's distance values for each variable of interest can create a lot of output and may be very difficult for interpretation later. Although we can return to using the `gridExtra` package to create a plot of plots, we'll use `cowplot` instead. The package we use in this case is purely a matter of personal preference. This situation is pretty common when using R. In fact, many packages can produce similar, if not identical, output.

In this case, we are choosing to use `cowplot` over `gridExtra` simply as a matter of personal preference. Were our goal to produce the boxplots above, we would use `cowplot`. This is because `cowplot` has functions such as `insert_*axis_grob` which allow multiple plots to use the same axes as the main plot. These functions aren't necessary for plots that can stand alone. Because we are creating a plot of plots that can stand alone, a goal that both the `gridExtra` and `cowplot` packages can satisfy, our package choice is mostly irrelevant.

```{r}
AgeGroup_no_log <- ggplot(data=AnalysisDF, aes(x=AgeGroup,y=Cooks,fill=AgeGroup)) +
  geom_boxplot() +
  facet_grid(Gender~Race) + 
  ggtitle(label = "Cooks Distance", subtitle = "By Age Group")+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.x = element_blank())

AgeGroup_log <- ggplot(data=AnalysisDF, aes(y=log(Cooks),fill=AgeGroup)) +
  geom_boxplot() +
  facet_grid(Gender~Race) + 
  ggtitle(label="log(Cooks Distance)", subtitle = "By Age Group")+
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        axis.title.x = element_blank())

plot_grid(AgeGroup_no_log,AgeGroup_log, labels=c("A","B"))
```
...
Because of the complexity of our model, a lot of output is produced. However, the output is a bit messy. 

Let's use `pander()` from the `pander` package to neatly organize our output.
