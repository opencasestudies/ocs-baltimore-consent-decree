...
```{r}
SignalDF <- AnalysisDF %>% filter(Year==2015:2017,Gender=="Female",Race=="White",AgeGroup=="20to24") %>% group_by(ArrestMonth,Race,Gender,AgeGroup) %>% mutate(r=Arrests/Population) %>% select(ArrestMonth,Race,Gender,AgeGroup,Arrests,Population,r)
...
ggplot(SignalDF[ntile(SignalDF$r,4)==4,],aes(x=Arrests,y=r))+
  geom_point()

ggplot(theoretical_rate_by_arrests)+
  geom_point(aes(x=theoretical_arrests, y=theoretical_rate))
```
