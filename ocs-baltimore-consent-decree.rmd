...
**For our analysis, we will focus on determining the extent of arrest disparities, if any, and whether the consent decree had an effect on these disparities.** *Correct this: this analysis is more about assessing whether the consent decree had an effect*
...
Weekdays cannot be linearly plotted, meaning we need to examine arrest counts by weekdays separately. Again, we will exlude 2019. 

```{r}
AnalysisDF %>% filter(!(Year %in% 2019)) %>% group_by(ArrestWeekday,Race,Gender,AgeGroup) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x=ArrestWeekday,y=n,fill=AgeGroup))+
...
```{r}
arrests_n_per_day <- AnalysisDF %>% group_by(ArrestDay) %>% summarise(n = sum(Arrests))

ggplot(data = arrests_n_per_day,
       mapping = aes(sample = n)) + stat_qq(distribution = stats::qpois, dparams = list(lambda = mean(arrests_n_per_day$n)),size=2, alpha=0.1) + 
...
It looks like our counts per day do not fit the distribution
...
```{r}
MODELB1 <- glm(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV, data=AnalysisDF, family=poisson, offset(log(Population)))
...
MODELB2 <- glm.nb(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV, data=AnalysisDF, offset(log(Population)))

AIC_B <- AIC(MODELB2)
```

```{r}
MODELC1 <- glm(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday, data=AnalysisDF, family=poisson, offset(log(Population)))
...
MODELC2 <- glm.nb(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday, data=AnalysisDF, offset(log(Population)))

AIC_C <- AIC(MODELC2)
```

```{r}
MODELD1 <- glm(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestQuarter_IV, data=AnalysisDF, family=poisson, offset(log(Population)))
...
MODELD2 <- glm.nb(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestQuarter_IV, data=AnalysisDF, offset(log(Population)))

AIC_D <- AIC(MODELD2)
```

```{r}
MODELE1 <- glm(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestSeason_IV, data=AnalysisDF, family=poisson, offset(log(Population)))
...
MODELE2 <- glm.nb(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestSeason_IV, data=AnalysisDF, offset(log(Population)))

AIC_E <- AIC(MODELE2)
```

It looks like our season indicator variable better explains the data

```{r}
MODELF1 <- glm(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestSeason_IV + ArrestMonth_IV, data=AnalysisDF, family=poisson, offset(log(Population)))
...
MODELF2 <- glm.nb(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestSeason_IV + ArrestMonth_IV, data=AnalysisDF, offset(log(Population)))

AIC_F <- AIC(MODELF2)
```

```{r}
MODELG1 <- glm(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestSeason_IV + ArrestMonth_IV + ArrestWeek_IV, data=AnalysisDF, family=poisson, offset(log(Population)))
...
MODELG2 <- glm.nb(Arrests ~ Gender + AgeGroup + Race + ArrestDay + CD_lin_spline + ArrestYear_IV + ArrestWeekday + ArrestSeason_IV + ArrestMonth_IV + ArrestWeek_IV, data=AnalysisDF, offset(log(Population)))
...
`r round((exp(summary(MODELG2)$coefficients["CD_lin_spline","Estimate"])*1000)-1000,2)` more arrests after the consent decree
