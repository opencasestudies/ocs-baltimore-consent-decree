...
Now we work on importing the data from our two sources.

### Baltimore arrest data

We can download arrest data from the Baltimore Police Department from through Open Baltimore [here](https://data.baltimorecity.gov/Public-Safety/BPD-Arrests/3i3v-ibrt) by clicking on "Export" on the menu bar in the upper right and selecting "CSV".  This will downloads a comma separated values (.csv) file named `BDP_Arrests.csv`.  Our case study works with a version of this data set that was last updated on September 3, 2019.  You can find this version in the Github repository associated with this case study.  If you use a more recent version of this arrest data, your results may not match ours exactly!

First we read in the data using `read_csv()` from the `readr` package:
```{r}
BPD_Arrests <- read_csv("BPD_Arrests.csv")
```

Notice the number of parsing failures!  The `read_csv()` function predicts a data type for each column of the dataset using the first 1000 rows of the data file and then reads in the data according to these predicted types.  However, sometimes values later in the data file don't match the data type that has been predicted!

Before we dive into the parsing errors, let's take a closer look at our data.  What variables are in the data set?  We can see the names of the variable columns here:
```{r}
names(BPD_Arrests)
```

While some of these variable names (like `Age`, `Sex`, and `Race`) seem obvious, others are not so clear.  For example, what is the variable `Post`?  Let's look at the first few rows of the data to get a better sense of these variables:
```{r}
head(BPD_Arrests, n = 10) %>% as.data.frame()
```

Using the `head()` function, we can see the first 10 rows of the data; piping into `as.data.frame()` allows us to see all of the columns rather than just the ones that will print nicely on the screen as a tibble.

Now we can see that we have some demographic information about the person arrested (`Age`, `Sex`, `Race`) as well as some information about the date/time of the arrest (`ArrestDate`, `ArrestTime`), information about the location of the arrest (`ArrestLocation`, `IncidentLocation`, `District`, `Post`, `Neighborhood`, `Latitude`, `Longitude`, `Location 1`), and information about the nature of the crime/arrest (`IncidentOffense`, `Charge`, `ChargeDescription`).  There is also some sort of arrest identification number given for each arrest (`Arrest`).  

Now back to the parsing failures we saw earlier.  Let's see if we can figure out where the problems are occurring! The `problems()` function allows us to retrieve information on the parsing failures we see above. 

```{r}
problems(BPD_Arrests)
```

We see that the `problems()` function returns a tibble where each row is a parsing failure.  For each parsing failure, we are given the row number where the parsing error occurred (`row`), the name of the column in the data where it occurred (`col`), and the type of value that was expected by the parser (`expected`) as well as the value that actually occurred (`actual`).  So, for example, we see that in the 10,626th row of the data file the parser expected a double in the `Post` column but instead saw the value `ETC.` which is clearly not a double! 

Let's look at which columns in the data file seem to be causing the problems by counting the number of errors for each value of `col`:
```{r}
problems(BPD_Arrests) %>%
  count(col)
```

The parsing errors all occur in the `Latitude`, `Longitude`, and `Post` columns, which are all variables associated with the location of the arrest.  
```{r}
problems(BPD_Arrests) %>%
  count(col, expected)
```
It also appears that in all but one of the cases, the failure occurred because the parser was expected a double (a number) and instead saw something that looked like a character type.

For our analysis, we won't be considering the location data, so these parsing failures may not matter for the variables that we need for our analysis.  However, it is still worth considering if there's an issue with the data file that might affect the integrity of the variables we do want to use!

Let's look more closely at the rows of data where these failures occurred:
```{r}
error_rows <- problems(BPD_Arrests)$row

BPD_Arrests %>%
  filter(row_number() %in% error_rows) %>%
  as.data.frame()
```
We see that there don't seem to be any issues with the variables in any of these rows, and that's because the places where the parsing failed have been filled in with `NA` values.  

We could consider changing the parsing type for these three columns from double to character to allow the parser to read in strings rather than numbers.  We can do this by specifying the `col_types` within the `read_csv()` function:
```{r}
BPD_Arrests <- read_csv("BPD_Arrests.csv", 
                        col_types = cols(
                          Post = col_character(),
...
```
Now we don't have any parsing failures!  But let's look at those rows that were causing errors before to see if we can see what the problem was:
```{r}
BPD_Arrests %>%
  filter(row_number() %in% error_rows) %>%
  as.data.frame()
```
We see that in these rows, the `Longitude` variable actually seems to contain the values of the neighborhood where the arrest occurred, which should be captured in the `Neighborhood` variable.  We also see that the `Post` variable contains the district names which should be captured in the `District` variable.  And the `District` variable seems to contain leftover text from the `ChargeDescription` variable.  For example, the `ChargeDescription` for the first line in the above data frame should read "GAMING/CARDS DICE ETC." perhaps, but the "DICE" was shifted into the `District` column and the "ETC." was shifted into the `Post` column.  Similarly, the second line in the above data should have a `ChargeDescription` that reads "THEFT UNDER $10" but the last "0" was shifted inot the `District` column and then following columns were also shifted by one so that the `Neighborhood` ended up in the `Longitude` column.  Further investigation of the data file `BPD_Arrests.csv` confirms this happened at the raw data level.

**Leah:Michael** Let's add a screenshot of the .csv file showing rows 10626 and 21345 to demonstrate this.

The good news is that the all of the errors appear to happen after the `ChargeDescription` column, so that the variables we need for our analysis will be unaffected.  We can actually specify in the `read_csv()` function the columns we want to read into our dataframe, using `cols_only()` option for `cols_type`. We can also specify the data type for each of these variables.
...
Now we get a parsing failure for each row of the dataset that includes a date -- all 14,298 of them! We see this more closely here:
```{r}
problems(BPD_Arrests) %>%
  count(col, expected)
```

This is because `read_csv()` cannot read in our date data, `ArrestDate`, in its current form.  The default date format for the date parser is ISO8601, which contains dates in the format "2019-01-01".  Our arrest dates are given in the format "01/01/2019" instead.  We could specify this date format here in the `read_csv()` function, but instead we will simply read in the data as a character and change to the appropriate format later in the analysis when we do further work with the date variable.

In general, using the `character` class for data allows us to read in the data without error and then convert the variable to the appropriate class after the data import step. This is particularly useful when data is not in a default form that `read_csv()` recognizes.
...
No parsing failures!  We can also see this by seeing that the problem data frame is empty!
...
**Leah:Michael** The code chunk below is the one that takes all the time! I think we can do this in an easier way?
```{r}
