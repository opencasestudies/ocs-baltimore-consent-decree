...
++ Need to create Consent Decree variable

CONSIDER ggarange cowplot

Ask about spatial analysis

DF %>% filter(Year %>% 2014.2016) %>% ggplot(___,___)+
...
```{r}
AnalysisDF %>% group_by(week=floor_date(ArrestDate,"week")) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x = week, y=n))+
  geom_point()+
...
```{r}
day2 <- AnalysisDF %>% filter(!(Year %in% 2019)) %>% group_by(day=floor_date(ArrestDate,"day")) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x=day, y=n))+
  geom_line(color="red")
```

```{r}
week2 <- AnalysisDF %>% filter(!(Year %in% 2019)) %>% group_by(week=floor_date(ArrestDate,"week")) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x=week, y=n))+
  geom_line(color="purple")
```

```{r}
month2 <- AnalysisDF %>% filter(!(Year %in% 2019)) %>% group_by(month=floor_date(ArrestDate,"month")) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x=month, y=n))+
  geom_line(color="green")
```

```{r}
season2 <- AnalysisDF %>% filter(!(Year %in% 2019)) %>% group_by(season=floor_date(ArrestDate,"season")) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x=season, y=n))+
  geom_line(color="blue")
```

```{r}
quarter2 <- AnalysisDF %>% filter(!(Year %in% 2019)) %>% group_by(quarter=floor_date(ArrestDate,"quarter")) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x=quarter, y=n))+
  geom_line(color="pink")
```

```{r}
year2 <- AnalysisDF %>% filter(!(Year %in% 2019)) %>% group_by(year=floor_date(ArrestDate,"year")) %>% mutate(n=sum(Arrests)) %>% ggplot(aes(x=year, y=n))+
  geom_line(color="orange")
```

```{r}
grid.arrange(day2,week2,month2,season2,quarter2,year2,nrow=3,ncol=2,top="Plot 2")
```
