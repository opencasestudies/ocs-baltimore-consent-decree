...
cor(regress_predict$fitted.values, AnalysisDF$Arrests)
```

```{r}
observed_fitted <- as.data.frame(cbind(AnalysisDF$Arrests, regress_predict$fitted.values))
...
paste("The outliers identified make up about", round(length_cooks_2_sd*100/length(AnalysisDF$Cooks),2), "% of the observations in our data.")
```
 
```{r}
MODELF2$call

AnalysisDF[influential_points,] %>% group_by(Gender) %>%
  summarise(median_cooks=median(Cooks)) %>%
  ungroup() %>%
  ggplot(aes(x=Gender,y=median_cooks,fill=Gender)) +
  geom_bar(stat="identity")

AnalysisDF[influential_points,] %>% group_by(Race) %>%
  summarise(median_cooks=median(Cooks)) %>%
  ungroup() %>%
  ggplot(aes(x=Race,y=median_cooks,fill=Race)) +
  geom_bar(stat="identity")

AnalysisDF[influential_points,] %>% group_by(AgeGroup) %>%
  summarise(median_cooks=median(Cooks)) %>%
  ungroup() %>%
  ggplot(aes(x=AgeGroup,y=median_cooks,fill=AgeGroup)) +
  geom_bar(stat="identity") + 
  theme(axis.text.x.bottom = element_blank(),
        axis.ticks.x = element_blank())
  
AnalysisDF[influential_points,] %>% group_by(ArrestDate) %>%
  summarise(median_cooks=median(Cooks)) %>%
  ungroup() %>%
  ggplot(aes(x=floor_date(ArrestDate,"month"),y=median_cooks)) +
  geom_bar(stat="identity")

AnalysisDF[influential_points,] %>% group_by(ArrestWeekday) %>%
  summarise(median_cooks=median(Cooks)) %>%
  ungroup() %>%
  ggplot(aes(x=ArrestWeekday,y=median_cooks,fill=ArrestWeekday)) +
  geom_bar(stat="identity") + 
  theme(axis.text.x.bottom = element_blank(),
        axis.ticks.x = element_blank())

AnalysisDF[influential_points,] %>% group_by(ArrestMonth_IV) %>%
  summarise(median_cooks=median(Cooks)) %>%
  ungroup() %>%
  ggplot(aes(x=ArrestMonth_IV,y=median_cooks,fill=ArrestMonth_IV)) +
  geom_bar(stat="identity") + 
  theme(axis.text.x.bottom = element_blank(),
        axis.ticks.x = element_blank())
```
