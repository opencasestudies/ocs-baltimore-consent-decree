...
The dataframes for the Baltimore populations should all be identically formatted. We can confirm this by taking a look at across year and between year dimensions.
...
+ Geography and Population have NA values
...
```{r}
arrests_n_per_day_poisson <- AnalysisDF %>%
  group_by(ArrestDay) %>%
  summarise(n = sum(Arrests))

arrests_n_per_day_nb <- AnalysisDF %>%
  group_by(ArrestDay) %>%
  summarise(n = sum(Arrests))
...
poisson_sample <- poisson_sample <- poisson_sample %>%
  group_by(sample_poisson) %>%
  tally() %>%
...
Let us review the AIC values for our models. Let's create a table with the `kableExtra` package. 
...
kable(model_AICs_table) %>%
  ...
  add_header_above(c("Model AIC Values"=length(colnames(model_AICs_table))))
```
...
We need to determine whether the influential points, the points with high values of Cooks Distance, are because of some unmeasured confounder.


length(which(log(AnalysisDF$Cooks)> mean(log(AnalysisDF$Cooks))+3*sd(log(AnalysisDF$Cooks))))

length(which(log(AnalysisDF$Cooks)> mean(log(AnalysisDF$Cooks))+2.5*sd(log(AnalysisDF$Cooks))))

length(which(log(AnalysisDF$Cooks)> mean(log(AnalysisDF$Cooks))+2*sd(log(AnalysisDF$Cooks))))

influential_points <- which(log(AnalysisDF$Cooks)> mean(log(AnalysisDF$Cooks))+2*sd(log(AnalysisDF$Cooks)))

length_cooks_2_sd <- length(which(log(AnalysisDF$Cooks)> mean(log(AnalysisDF$Cooks))+2*sd(log(AnalysisDF$Cooks))))

paste("The outliers identified make up about",
      round(length_cooks_2_sd*100/length(AnalysisDF$Cooks),2),
      "% of the observations in our data.")

```{r}
AnalysisDF <- AnalysisDF %>%
  ungroup() %>%
  mutate(logCooks = log(Cooks),
         diff_logCooks=abs(logCooks-mean(logCooks)),
         threshold=sd(logCooks)*4,
         Type=as.factor(ifelse(diff_logCooks>threshold,
...
```{r}
AnalysisDF %>%
  group_by(Type,ArrestMonth) %>%
  summarise(count=n()) %>%
  mutate(prop=prop.table(count)) %>%
  ggplot(aes(x=ArrestMonth,y=prop, fill=prop)) +
  geom_bar(stat="identity") +
  facet_grid(.~Type)
```
...
resid_check %>%
  ...
  geom_point(alpha=1/50,size=0.5,color="gray")+
  geom_smooth(size=2) +
  xlab("Fitted Value") +
  ylab("Residual") + 
...
resid_check %>%
  ggplot(aes(x=fitted.value, y=residual)) +
  geom_point(alpha=1,size=0.5,color="gray")+
