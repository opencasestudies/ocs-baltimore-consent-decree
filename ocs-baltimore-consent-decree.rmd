...
observed_fitted <- observed_fitted %>% rename("Line of Equality"= perfect_fit, "Observed | Model F"=Modeled) %>% gather(key = "Data", value = "Observed", -Expected)

ggplot(observed_fitted, aes(x = Expected, y = Observed, color=Data)) + 
...
```{r}
which(log(AnalysisDF$Cooks)>= mean(log(AnalysisDF$Cooks))+3*sd(log(AnalysisDF$Cooks)))

which(log(AnalysisDF$Cooks)>= mean(log(AnalysisDF$Cooks))+2*sd(log(AnalysisDF$Cooks)))

length(which(log(AnalysisDF$Cooks)>= mean(log(AnalysisDF$Cooks))+2*sd(log(AnalysisDF$Cooks))))/length(AnalysisDF$ArrestDay)

AnalysisDF_no_influential_points <- AnalysisDF[-which(log(AnalysisDF$Cooks)>= mean(log(AnalysisDF$Cooks))+2*sd(log(AnalysisDF$Cooks))),]
```

```{r}
MODELF3 <- glm.nb(Arrests ~ Gender + AgeGroup + ArrestDay + ArrestWeekday + ArrestMonth_IV + Race * CD_lin_spline, data=AnalysisDF_no_influential_points, offset(log(Population)))
```

```{r}
regress_predict2 <- lm(AnalysisDF_no_influential_points$Arrests ~ MODELF3$fitted.values)
summary(regress_predict2)
cor(regress_predict2$fitted.values, AnalysisDF_no_influential_points$Arrests)
observed_fitted2 <- as.data.frame(cbind(AnalysisDF_no_influential_points$Arrests, regress_predict2$fitted.values))
colnames(observed_fitted2) <- c("Expected","Modeled")
observed_fitted2$perfect_fit <- observed_fitted2$Expected

observed_fitted2 <- observed_fitted2 %>% rename("Line of Equality"= perfect_fit, "Observed | Model F"=Modeled) %>% gather(key = "Data", value = "Observed", -Expected)
```

```{r}
MODELF3_fit_plot <- ggplot(observed_fitted2, aes(x = Expected, y = Observed, color=Data)) +
  geom_point(alpha=1/100) +
  geom_smooth(size=2) +
  ggtitle("Fit, Model F3") +
  theme_minimal()

top_density <- axis_canvas(MODELF3_fit_plot, axis = "x") +
  geom_density(data=observed_fitted2, aes(x=Expected,alpha=0.25)) +
  theme_void()

right_density <- axis_canvas(MODELF3_fit_plot, axis = "y",coord_flip = TRUE) + 
  geom_density(data=observed_fitted2, aes(x=Observed,fill=Data,alpha=0.25)) +
  theme_void() + 
  coord_flip()

final_model_fit <- insert_xaxis_grob(MODELF3_fit_plot, top_density, position = "top")

final_model_fit <- insert_yaxis_grob(final_model_fit, right_density, position = "right")

ggdraw(final_model_fit)
