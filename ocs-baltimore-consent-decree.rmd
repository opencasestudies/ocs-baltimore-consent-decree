...
**Need to show graphical representation of small number problem that is intuitive, currently incorrect**

SignalDF <- AnalysisDF %>% filter(Year==2015:2017,Gender=="Female",Race=="White",AgeGroup=="20to24") %>% group_by(ArrestMonth,Race,Gender,AgeGroup) %>% mutate(r=Arrests/Population) %>% select(ArrestMonth,Race,Gender,AgeGroup,Arrests,Population)
...
```{r}
AnalysisDF$Month_IV <- as.factor(AnalysisDF$Month)
AnalysisDF$ArrestWeek_IV <- as.factor(AnalysisDF$ArrestWeek)
AnalysisDF$ArrestMonth_IV <- as.factor(AnalysisDF$ArrestMonth)
AnalysisDF$ArrestQuarter_IV <- as.factor(AnalysisDF$ArrestQuarter)
AnalysisDF$ArrestSeason_IV <- as.factor(AnalysisDF$ArrestSeason)
AnalysisDF$ArrestYear_IV <- as.factor(AnalysisDF$ArrestYear)
```
...
It looks like our quarter indicator variable better explains the data
