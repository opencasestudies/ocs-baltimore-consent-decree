...
Let's see if we can figure out where the problems are occurring! The `problems()` function allows us to retrieve information on the parsing failures we see above. We can determine which variable columns are causing problems (and how many problems there are) with the `summary()` function after some minor transformations.

Let us look at some of the parsing failures identified by `read_csv()`. Note that there may be other problems with the data that aren't identified as parsing failures; errors like these may require more direct methods to identify them. 

```{r}
problems(BPD_Arrests)
```

We see that the `problems()` function returns a tibble where each row is a parsing failure.  For each parsing failure, we are given the row number where the parsing error occurred (`row`), the name of the column in the data where it occurred (`col`), and the type of value that was expected by the parser (`expected`) as well as the value that actually occurred (`actual`).  So, for example, we see that in the 10,626 row of the data file the parser expected a double in the `Post` column but instead saw the value `ETC.` which is clearly not a double! 
...
The parsing errors all occur in the `Latitude`, `Longitude`, and `Post` columns.  What other variables are in the data set?
```{r}
names(BPD_Arrests)
```

From the variable names we can see that we have some demographic information about the person arrested. We also have some information about the nature of the arrest. Spatial data is also available. 
