...
**Michael**: Improve this table by writing out model formulas.
**Michael**: Consider adding a number of terms column

```{r}
kable(model_AICs_table, align = "c") %>%
  ...
  add_header_above(c("Models A-F"=length(colnames(model_AICs_table))))
```
...
It appears that a few outlier points may be very influential to our model. The third quartile of expected arrests is predicted explained well by the model. Fitted values outside of this range are either underestimates or overestimates. 
...
AnalysisDF <- AnalysisDF %>%
  ungroup() %>%
  mutate(diff_Cooks=abs(log(Cooks)-mean(log(Cooks))),
         threshold=sd(log(Cooks))*3,
...
Influential points, per our definition, form about `r round((as.numeric(summary(AnalysisDF$Type)[1])/as.numeric(summary(AnalysisDF$Type)[2]))*100,2)`% of our observations. 
...
**Michael**: This might need a better title.

```{r}
grid.arrange(A,B, ncol=2,top="Influential Points")
grid.arrange(C,D, ncol=2, top="Influential Points (Continued)")
```

## Residual diagnostics

**Michael**: explain what you are doing here 
...
**Michael**: This summary needs to be about two paragraphs long. The first should provide numerical results. A sentence or two should discuss the "Final Plot". THe second paragraph should make known limitations of our analysis. 
...
**Michael**: Sensitivity analysis without influential points 
